package com.jaboumal.services;

import com.jaboumal.util.ConfigUtil;
import com.jaboumal.util.DateUtil;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;

import java.io.File;

import static org.junit.jupiter.api.Assertions.*;

class PDFServiceTest {

    private PDFService pdfService;

    @BeforeEach
    void setUp() {
        ConfigUtil.loadConfigFile();
        pdfService = new PDFService();
    }

    @Test
    void createPDFGewehr() {
        String barcode = "";
        String outputFile = pdfService.createPDFGewehr("Hans", "Peter", DateUtil.stringToDate("01.01.2000"), barcode, true);

        File file = new File(String.format(outputFile, 1));

        assertEquals("src/test/resources/output/Berchtoldschiessen_Hans_Peter_1.png", outputFile);
        assertNotNull(file);
        assertTrue(file.exists());

        file.delete();
    }

    @Test
    void createPDFPistole() {
        String outputFile = pdfService.createPDFPistole("Hans", "Peter", DateUtil.stringToDate("01.01.2000"), true);

        File file = new File(String.format(outputFile, 1));

        assertEquals("src/test/resources/output/Berchtoldschiessen_Hans_Peter_2.png", outputFile);
        assertNotNull(file);
        assertTrue(file.exists());

        file.delete();
    }

}